function jobdeer_post(path, params, method) {
...
function jobdeer_send2get()
{
  jobdeer_post( post_url , { 'title':document.title , 'url':window.location.href , 'content':replace_all_rel_by_abs(document.getElementById('readInner').innerHTML) } );
}
	...
	init: function() {
		...
		if(!articleContent) {
			...
			articleContent.innerHTML = [
				"<p>抱歉快速发布无法分析页面内容。<a href='"+post_url+"' target='_blank'>请点击网站的投稿链接</a>，手工复制内容粘贴到编辑器继续。"
			].join('');
		...
		var jobdeerDiv       = document.createElement("DIV");
		jobdeerDiv.id        = "jobdeer";
		jobdeerDiv.innerHTML = '提交到OneBlog';
		...
		//innerDiv.appendChild( articleFooter  );
		 ...
		 overlay.appendChild( jobdeerDiv       );
		...
		**/
		if((window.location.protocol + "//" + window.location.host + "/") === window.location.href)
		{
			...
			var rootWarning = document.createElement('p');
				...
				rootWarning.innerHTML = "<em>快速发布</em> 主要用于文章页面，而非列表页 " +
					"如果一定要继续 <a onClick='javascript:document.getElementById(\"readability-warning\").style.display=\"none\";document.getElementById(\"readability-content\").style.display=\"block\";'>点这里</a> 继续。";
...
readability.init();
document.getElementById('jobdeer').addEventListener( 'click' , jobdeer_send2get , false );
